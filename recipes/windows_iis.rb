...
execute 'create_golf_app_pool' do
  command %(C:\\Windows\\System32\\inetsrv\\appcmd.exe add apppool /name:"#{node['golf_app']['app_pool_name']}" /managedRuntimeVersion:"v4.0" /processModel.identityType:ApplicationPoolIdentity)
  not_if %(C:\\Windows\\System32\\inetsrv\\appcmd.exe list apppool "#{node['golf_app']['app_pool_name']}" >nul 2>&1)
...
execute 'stop_default_website' do
  command %(C:\\Windows\\System32\\inetsrv\\appcmd.exe stop site "Default Web Site")
  only_if %(C:\\Windows\\System32\\inetsrv\\appcmd.exe list site "Default Web Site" /state:Started >nul 2>&1)
  only_if { ::File.exist?('C:\\Windows\\System32\\inetsrv\\appcmd.exe') }
  ignore_failure true
end
...
execute 'remove_existing_golf_site' do
  command %(C:\\Windows\\System32\\inetsrv\\appcmd.exe delete site "#{node['golf_app']['site_name']}")
  only_if %(C:\\Windows\\System32\\inetsrv\\appcmd.exe list site "#{node['golf_app']['site_name']}" >nul 2>&1)
  only_if { ::File.exist?('C:\\Windows\\System32\\inetsrv\\appcmd.exe') }
  ignore_failure true
end
...
if node['golf_app']['enable_ssl']
  ...
  execute 'remove_existing_golf_ssl_site' do
    command %(%windir%\\system32\\inetsrv\\appcmd delete site "#{node['golf_app']['site_name']}-SSL")
    only_if %(%windir%\\system32\\inetsrv\\appcmd list site "#{node['golf_app']['site_name']}-SSL")
    ignore_failure true
  end

  # Create SSL site using appcmd
  execute 'create_golf_ssl_website' do
    command %(%windir%\\system32\\inetsrv\\appcmd add site /name:"#{node['golf_app']['site_name']}-SSL" /physicalPath:"#{node['golf_app']['web_root']}" /bindings:https/*:#{node['golf_app']['ssl_port']}: /applicationDefaults.applicationPool:"#{node['golf_app']['app_pool_name']}")
    not_if %(%windir%\\system32\\inetsrv\\appcmd list site "#{node['golf_app']['site_name']}-SSL")
    retries 3
    retry_delay 5
  end
end
...
powershell_script 'configure_primary_website' do
  code <<-EOH
    ...
    #{'    '}
        ...
        Write-Host "Using appcmd.exe for site management..."
        & $appcmd stop site "Default Web Site" 2>$null
        & $appcmd start site "Elite Golf Site" 2>$null
        Write-Host "Site management completed using appcmd"
      }
    #{'  '}
  ...
  code <<-EOH
    ...
    #{'    '}
        ...
        if ($attempt -lt $maxAttempts) {
          ...
          Start-Sleep -Seconds 10
    #{'      '}
          # Check if site is still running (with fallback support)
          try {
            ...
            Write-Host "Cannot check site status (PowerShell cmdlets unavailable) - continuing..."
          }
        }
      }
    }
    ...
    #{'  '}
      ...
      try {
        ...
        # Fallback to appcmd if PowerShell cmdlets fail
        Write-Host "PowerShell cmdlets failed, using appcmd fallback..."
        $appcmd = "$env:SystemRoot\\System32\\inetsrv\\appcmd.exe"
        & $appcmd delete site "Elite Golf Site" 2>$null
        Start-Sleep -Seconds 2
        & $appcmd add site "/name:Elite Golf Site" "/bindings:http/*:#{node['golf_app']['port']}:" "/physicalPath:#{node['golf_app']['web_root']}"
        & $appcmd start site "Elite Golf Site"
      }
      Start-Sleep -Seconds 5
    #{'  '}
      # Final verification
